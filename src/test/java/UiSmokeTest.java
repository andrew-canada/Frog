...
final class UiSmokeTest {
    static void run(){
        ...
        try{SwingUtilities.invokeAndWait(()->{
            var list=new WashroomListViewModel();var reviews=new ReviewsViewModel();var map=new MapViewModel();var busyness=new BusynessViewModel();
            MainView main=new MainView(list,map);main.setWashrooms(List.of(new Washroom("w1","Bahen Centre Washroom",
            ...
            for(JPanel view:views){view.setSize(900,600);layoutTree(view);assertButtonContrast(view);BufferedImage image=new BufferedImage(900,600,BufferedImage.TYPE_INT_ARGB);view.paint(image.getGraphics());
    ...
    private static void assertButtonContrast(Container container){for(Component child:container.getComponents()){if(child instanceof JButton button&&button.getText()!=null&&!button.getText().isBlank()){TestSupport.check(Color.BLACK.equals(button.getForeground()),button.getText()+" should use black text");TestSupport.check(Color.WHITE.equals(button.getBackground()),button.getText()+" should use a white background");}if(child instanceof Container nested)assertButtonContrast(nested);}}
