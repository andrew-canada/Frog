...
final class LoginInteractorTest {
    static void run(){class Fake implements UserDataAccessInterface{User current;User saved=new User("demo", BCrypt.hashpw("secret", BCrypt.gensalt()), "");public Optional<User> get(String n){return Optional.of(saved);}public boolean existsByName(String n){return true;}public void save(User u){}public void setCurrentUser(User u){current=u;}public Optional<User> getCurrentUser(){return Optional.ofNullable(current);} public void removeUser(String n){}}
        ...
        class LegacyFake implements UserDataAccessInterface{User current;User saved=new User("legacy","secret","");public Optional<User> get(String n){return Optional.of(saved);}public boolean existsByName(String n){return true;}public void save(User u){saved=u;}public void setCurrentUser(User u){current=u;}public Optional<User> getCurrentUser(){return Optional.ofNullable(current);}public void removeUser(String n){}}
        LegacyFake legacy=new LegacyFake();new LoginInteractor(legacy,d->out[0]=d).execute(new LoginInputData("legacy","secret"));
