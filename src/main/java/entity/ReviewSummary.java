...
public record ReviewSummary(double averageRating, double averageCleanliness, int reviewCount) {
    ...
    public static ReviewSummary fromReviews(List<Review> reviews) {
        if (reviews.isEmpty()) return empty();
        return new ReviewSummary(reviews.stream().mapToDouble(Review::rating).average().orElse(0),
                reviews.stream().mapToDouble(Review::cleanliness).average().orElse(0), reviews.size());
    }
}
