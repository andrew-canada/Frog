...
public final class DBStatusReportDataAccessObject implements StatusReportDataAccessInterface {
    ...
    @Override public void save(StatusReport report) {
    ...
    @Override public List<StatusReport> getRecentForWashroom(String washroomId, LocalDateTime since) {
        return getForWashroom(washroomId, since, LocalDateTime.now().plusSeconds(1));
    }

    @Override public List<StatusReport> getForWashroom(String washroomId, LocalDateTime from, LocalDateTime to) {
    ...
    private StatusReport toEntity(Document document) {
        ...
        try { issue = MaintenanceIssue.valueOf(issueName); }
        catch (IllegalArgumentException ignored) { issue = MaintenanceIssue.OTHER; }
