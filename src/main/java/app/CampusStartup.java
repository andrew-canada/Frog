...
final class CampusStartup {
    ...
    static void seedInitialData(final DBWashroomDataAccessObject washrooms, final DBReviewDataAccessObject reviews,
                                final DBStatusReportDataAccessObject reports, final Set<String> names) {
        washrooms.ensurePerformanceIndexes();
        reviews.ensurePerformanceIndexes();
        reports.ensurePerformanceIndexes();
        final List<Washroom> jsonWashrooms = washrooms.getByNames(names);
