...
public final class LoginView extends JPanel{
    ...
    public LoginView(LoginViewModel model,LoginController controller) {
        ...
        JButton login=Theme.primary("Log in"),signup=Theme.button("Create account"),back=Theme.button("Back to map");
        ...
        card.add(login);card.add(Box.createVerticalStrut(8));
        card.add(signup);card.add(Box.createVerticalStrut(8));
        card.add(back);card.add(Box.createVerticalStrut(16));
        card.add(message);add(card);
