...
public final class ReportReviewDialog extends JDialog {
    ...
    public ReportReviewDialog(Window owner, ReportReviewController controller,
        ...
        for (String reason : REASONS) {
            JCheckBox box = new JCheckBox(reason);
        ...
        JTextArea details = new JTextArea(3, 26);
        ...
        JButton submit = Theme.primary("Submit");
        buttons.add(cancel);
        buttons.add(submit);
        page.add(buttons);
        ...
        PropertyChangeListener listener = ev -> {
            ReportReviewViewModel.State state = model.getState();
            if (state.submitted()) {
