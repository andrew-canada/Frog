...
public final class RecommendWashroomInteractor implements RecommendWashroomInputBoundary {
    ...
    @Override
    public void execute(RecommendWashroomInputData in) {
        ...
        if (candidates.isEmpty()) {
            presenter.presentError("No washrooms match those preferences");
            return;
        }
    ...
    private Scored score(Washroom w, RecommendWashroomInputData in) {
        ...
        parts.put("rating", rating * wr);
        parts.put("proximity", proximity * wp);
        parts.put("quiet now", quiet * wq);
        parts.put("preference match", match * wm);
