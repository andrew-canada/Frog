...
public final class SubmitStatusReportInteractor implements SubmitStatusReportInputBoundary {
    private final StatusReportDataAccessInterface reports; private final SubmitStatusReportOutputBoundary presenter; private final Clock clock;
    ...
    public SubmitStatusReportInteractor(StatusReportDataAccessInterface reports, SubmitStatusReportOutputBoundary presenter, Clock clock) {
        this.reports=reports; this.presenter=presenter; this.clock=clock;
    }
    @Override public void execute(SubmitStatusReportInputData in) {
        if (in.busyness()<1 || in.busyness()>5 || in.cleanliness()<1 || in.cleanliness()>5) {
            presenter.present(new SubmitStatusReportOutputData(false, 0, "Choose values from 1 to 5")); return;
        }
