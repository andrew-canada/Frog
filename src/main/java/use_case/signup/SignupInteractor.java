...
public final class SignupInteractor implements SignupInputBoundary {
    ...
    public SignupInteractor(UserRepository users, CurrentUserSession session, PasswordHasher passwords,
    ...
    public void execute(SignupInputData input) {
        ...
        if (name.length() < 3 || input.password() == null || input.password().length() < 4) {
            presenter.present(new SignupOutputData(false, null, "Use 3+ characters for the name and 4+ for the password"));
            return;
        }
